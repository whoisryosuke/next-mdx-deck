...
Note that this isn't "best practice" storing gamepad input into state (since it happens up to 60fps).
...
Note that each button on a gamepad is actually a `GamepadButton` class.

It has the `pressed` boolean property which we use.
...
- **Don't** store gamepad input into state.
- **Debounce** input where possible, unless you need low latency interactions.
- **Memoize and callback** all functions inside the "game loop"
...
Standard keyboards and mice can pose difficulties for many people – especially users with physical, sensory, or cognitive challenges – and there are lots of other options available.
